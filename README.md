...
### __resolve_reference
* Each type which is decorated with @key or @extend is added to Entity union
...
* You should define `__resolve_reference`, if you need to extract object before passing it to fields resolvers (example: integration_tests.service_b.schema.FileNode)
* You should not define `__resolve_reference`, if fileds resolvers need only data passed in fieldset (example: integration_tests.service_a.schema.FunnyText)
